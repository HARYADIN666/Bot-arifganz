	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				wait2: '*[❗] WAIT SEDANG PROSES*',
				wait3: '*[❗] Sabar om...pasti kemren nih*',
				wait4: '*[❗] Sabar dulu sedang proses....*',
				success: 'Berhasil ✔️',
				error: {
			...
			const ownerNumber = ["6282279601471@s.whatsapp.net"] // Ganti pake nomor lu
			const ramlanNumber = ["6282279601471@s.whatsapp.net"] // JANGAN DI UBAH NGENTOD
			...
			switch(command) {
               ...
               client.sendMessage(from, '*Nih nomor ownerku hehee😁 jangan di galakin ya:)*',MessageType.text, { quoted: mek} )
                break
				case 'ocr':
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('Yang mau di tulis apaan om..?')
					ramlan = body.slice(7)
					reply(mess.wait)
				...
				case 'bc':
					...
					if (!isRamlan) return reply('Kamu Haryadin?')
                ...
                case 'hidetag':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ greply stickernya um')
					reply(mess.wait)
...
case 'thundername':
					...
					client.sendMessage(from, ramlan, image, {quoted: mek, caption: 'Nih om:v'})
					break
//MENU COMLI
